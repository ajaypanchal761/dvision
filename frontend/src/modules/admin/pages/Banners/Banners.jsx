...
const Banners = () => {
  ...
  const fetchBanners = async () => {
    try {
      ...
      if (response.success && response.data?.banners) {
        ...
        const mappedBanners = response.data.banners.map(banner => ({
          _id: banner._id,
          id: banner._id,
          title: banner.title || '',
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await bannerAPI.delete(deleteBannerId)
      if (response.success) {
        await fetchBanners()
      ...
      <div className="px-4 sm:px-6 lg:px-8 pt-3 sm:pt-4 md:pt-5 lg:pt-6">
        ...
        <div className="pb-0">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-white">Manage Banners</h1>
            </div>
            <button
        ...
        <div className="mt-3 sm:mt-4 md:mt-5 lg:mt-6">
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-4 lg:gap-6">
            <div className="bg-white rounded-lg shadow-md p-2.5 sm:p-3 md:p-4 border border-gray-200">
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-gray-500 text-xs font-medium">Total Banners</p>
                  <p className="text-base sm:text-lg md:text-xl font-bold text-gray-900 mt-0.5">{banners.length}</p>
                </div>
            ...
            <div className="bg-gray-50 rounded-lg shadow-md p-2.5 sm:p-3 md:p-4 border border-gray-100">
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-gray-500 text-xs sm:text-sm font-medium">Active Banners</p>
                  <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-gray-900 mt-0.5 sm:mt-1">{banners.filter(b => b.status === 'Active').length}</p>
                </div>
            ...
            <div className="bg-gray-50 rounded-lg shadow-md p-2.5 sm:p-3 md:p-4 border border-gray-100">
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-gray-900 mt-0.5 sm:mt-1">{filteredBanners.length}</p>
                </div>
        ...
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden mt-3 sm:mt-4 md:mt-5 lg:mt-6">
          ...
          <div className="p-3 sm:p-4 border-b border-gray-200">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search banners by title, description, or status..."
                value={searchTerm}
          ...
          <div className="overflow-x-auto">
            {isLoading ? (
              <div className="p-6 sm:p-8 text-center">
                ...
                <p className="mt-2 sm:mt-3 text-gray-500 text-xs sm:text-sm">Loading banners...</p>
              </div>
            ) : (
                    ...
                    <tr>
                      <td colSpan="8" className="px-3 sm:px-4 py-6 sm:py-8 text-center">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-gray-500 font-medium text-xs sm:text-sm">{searchTerm ? 'No banners found matching your search.' : 'No banners found.'}</p>
                          <p className="text-gray-400 text-[10px] sm:text-xs mt-1">{searchTerm ? 'Try adjusting your search criteria.' : 'Click "Add Banner" to create one.'}</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
