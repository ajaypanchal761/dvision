  ...
  const fetchTeachers = async (page = 1) => {
    try {
      ...
      if (response.success && response.data?.teachers) {
        ...
        const mappedTeachers = response.data.teachers.map(teacher => ({
          _id: teacher._id,
          id: teacher._id,
          name: teacher.name || '',
          email: teacher.email || '',
      ...
      <div className="px-4 sm:px-6 lg:px-8 pt-3 sm:pt-4 md:pt-5 lg:pt-6">
        ...
        <div className="pb-0">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-white">Manage Teachers</h1>
            </div>
            <button
          ...
          <div className="overflow-x-auto">
            {isLoading ? (
              <div className="p-6 sm:p-8 md:p-10 text-center">
                ...
                <p className="mt-2 sm:mt-3 text-gray-500 text-xs sm:text-sm">Loading teachers...</p>
              </div>
            ) : (
                    ...
                    <tr>
                      <td colSpan="6" className="px-3 sm:px-4 py-6 sm:py-8 text-center">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-gray-500 font-medium text-xs sm:text-sm">{searchTerm ? 'No teachers found matching your search.' : 'No teachers found.'}</p>
                          <p className="text-gray-400 text-[10px] sm:text-xs mt-1">{searchTerm ? 'Try adjusting your search criteria.' : 'Click "Add Teacher" to create one.'}</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    filteredTeachers.map((teacher) => (
                      <tr key={teacher._id || teacher.id} className="hover:bg-gray-50 transition-all duration-200">
                        <td className="px-2 sm:px-3 py-2 whitespace-nowrap text-center">
                          <div className="flex items-center justify-center">
                            {teacher.image || teacher.imagePreview ? (
                              <img
                                src={teacher.imagePreview || teacher.image}
                                alt={teacher.name}
                        ...
                        <td className="px-2 sm:px-3 py-2 whitespace-nowrap text-center">
                          <div className="text-xs font-semibold text-gray-900">{teacher.name}</div>
                        </td>
                        <td className="px-2 sm:px-3 py-2 whitespace-nowrap hidden sm:table-cell text-center">
                          <div className="text-xs text-gray-600 truncate max-w-[150px] mx-auto">{teacher.email || 'N/A'}</div>
                        </td>
                        <td className="px-2 sm:px-3 py-2 whitespace-nowrap hidden md:table-cell text-center">
                          <div className="text-xs text-gray-600">{teacher.mobile}</div>
                        </td>
                        ...
                        <td className="px-2 sm:px-3 py-2 whitespace-nowrap text-center text-xs font-medium">
                          <div className="flex items-center justify-center space-x-1 sm:space-x-2">
                            ...
                            <button
                              onClick={() => handleDeleteClick(teacher._id || teacher.id)}
