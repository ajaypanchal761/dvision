...
const Attendance = () => {
      ...
      const defaultAttendance = [
        {
          ...
          attendanceDate: '2024-12-03',
          checkInTime: '09:00',
          checkOutTime: '17:00',
          createDateTime: '2024-12-03 08:55:00',
        },
        {
          id: 2,
          teacherImage: null,
          ...
          checkInTime: '09:15',
          checkOutTime: '17:30',
          createDateTime: '2024-12-03 09:10:00',
        },
      ]
  ...
  // Listen for storage changes (when add/edit pages update localStorage)
  useEffect(() => {
  ...
  const handleDeleteConfirm = () => {
    const updatedAttendance = attendance.filter(a => a.id !== deleteAttendanceId)
  ...
  const filteredAttendance = attendance.filter(record =>
    ...
    record.attendanceDate.toLowerCase().includes(searchTerm.toLowerCase()) ||
    record.checkInTime.toLowerCase().includes(searchTerm.toLowerCase()) ||
    record.checkOutTime.toLowerCase().includes(searchTerm.toLowerCase())
  )
      ...
      <div className="px-4 sm:px-6 lg:px-8 pt-3 sm:pt-4 md:pt-5 lg:pt-6">
        ...
        <div className="pb-0">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            ...
            <button
              onClick={() => navigate('/admin/attendance/add')}
            ...
            <div className="bg-white rounded-lg shadow-md p-2.5 sm:p-3 md:p-4 border border-gray-200">
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-base sm:text-lg md:text-xl font-bold text-gray-900 mt-0.5">{attendance.length}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-gray-900 mt-0.5 sm:mt-1">
                    {attendance.filter(a => a.attendanceDate === new Date().toISOString().slice(0, 10)).length}
                  </p>
                </div>
        ...
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden mt-3 sm:mt-4 md:mt-5 lg:mt-6">
          ...
          <div className="p-3 sm:p-4 border-b border-gray-200">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by teacher name, date, check-in or check-out time..."
                value={searchTerm}
                  ...
                  <tr>
                    <td colSpan="7" className="px-3 sm:px-4 py-6 sm:py-8 text-center">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-gray-500 font-medium text-xs sm:text-sm">{searchTerm ? 'No attendance records found matching your search.' : 'No attendance records found.'}</p>
                        <p className="text-gray-400 text-[10px] sm:text-xs mt-1">{searchTerm ? 'Try adjusting your search criteria.' : 'Click "Add Attendance" to create one.'}</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={record.id} className="hover:bg-gray-50 transition-all duration-200">
                      <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                        <div className="flex items-center">
                          {record.teacherImage || record.teacherImagePreview ? (
                            <img
                              src={record.teacherImagePreview || record.teacherImage}
                              alt={record.teacherName}
                              className="h-6 w-6 sm:h-8 sm:w-8 rounded-full object-cover ring-2 ring-gray-200"
                            />
                          ) : (
                      ...
                      <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                        <div className="text-xs font-semibold text-gray-900">{record.teacherName}</div>
                      </td>
                      ...
                      <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                        <div className="text-xs font-semibold text-green-700 bg-green-50 px-2 py-0.5 rounded-lg inline-block">
                          {record.checkInTime}
                        </div>
                      </td>
                      <td className="px-2 sm:px-3 py-2 whitespace-nowrap hidden md:table-cell">
                        <div className="text-xs font-semibold text-red-700 bg-red-50 px-2 py-0.5 rounded-lg inline-block">
                          {record.checkOutTime}
                        </div>
                      </td>
                      <td className="px-2 sm:px-3 py-2 whitespace-nowrap hidden lg:table-cell">
                        <div className="text-[10px] sm:text-xs text-gray-500">{record.createDateTime}</div>
                      </td>
                      <td className="px-2 sm:px-3 py-2 whitespace-nowrap text-right text-xs font-medium">
                        <div className="flex items-center justify-end space-x-1 sm:space-x-2">
                          <button
                            onClick={() => navigate(`/admin/attendance/edit/${record.id}`)}
