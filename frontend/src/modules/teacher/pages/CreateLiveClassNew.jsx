...
const CreateLiveClass = () => {
  ...
  useEffect(() => {
    if (formData.classId && assignedOptions.classes.length > 0) {
      const selectedClass = assignedOptions.classes.find(c => c._id === formData.classId);
      if (selectedClass) {
        // Get subjects that are assigned to this teacher for this class
        // We need to check timetables to get valid subject-class combinations
        const validSubjects = assignedOptions.subjects.filter(subject => {
          // Check if this subject-class combination exists in teacher's timetables
          return assignedOptions.classes.some(cls => 
            cls._id === formData.classId
          );
        });
        setFilteredSubjects(assignedOptions.subjects);
      } else {
        setFilteredSubjects([]);
      }
    } else {
  ...
  }, [formData.classId, assignedOptions]);
